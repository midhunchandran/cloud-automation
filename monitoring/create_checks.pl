...
usage: $0 -a accountId -e entityId

  -a        : Rackspace account number

  example: $0 -a 34536 -e eNud239nh
...
sub create_pingcheck_request {

    my @monitoring_zones_poll;
    $monitoring_zones_poll[0] = "mzord";
    my $request = {};
    $request->{"label"} = "ping";
    $request->{"target_alias"} = "default";
    $request->{"period"} = 30;
    $request->{"timeout"} = 10;
    $request->{"monitoring_zones_poll"} = \@monitoring_zones_poll;
    $request->{"type"} = "remote.ping";
    
    return $request;

}

sub create_agentcpu_request {

    my @monitoring_zones_poll;
    $monitoring_zones_poll[0] = "mzord";
    my $request = {};
    $request->{"label"} = "agentcpu";
    #$request->{"target_alias"} = "default";
    $request->{"period"} = 30;
    $request->{"timeout"} = 10;
    #$request->{"monitoring_zones_poll"} = \@monitoring_zones_poll;
    $request->{"type"} = "agent.cpu";
...
sub list_entities {
    my $req_scalar = create_agentcpu_request();
    ...
    $client->POST("$rack_accountId/entities/$entityId/checks", $req_json);

    if($client->responseCode() eq '200' || $client->responseCode() eq '201' || $client->responseCode() eq '202') {
        my $resp = $client->responseContent();
        my $resp_parsed = decode_json($resp);
        printf "Entities: %s \n", $resp;
