...
my $rack_database_url = "https://dfw.databases.api.rackspacecloud.com/v1.0";
...
sub create_database {
    ...
    $client->POST("$rack_accountId/instances", $req_json);
    if($client->responseCode() eq '200' || $client->responseCode() eq '202') {
        my $resp = $client->responseContent();
        print "$resp\n";
        my $resp_parsed = decode_json($resp);
        $db_hostname = $resp_parsed->{"instance"}->{"hostname"}; 
        printf "Database created with hostname: %s \n", $db_hostname;
        return 0;
    } else {
...
sub update_chef_role {
    my @args = ("/home/midhun/projects/chef-api/set_site_and_db", "-s", "demo.cloudmovers.org", "-d", $db_hostname);
    system(@args) == 0 or die "system @args failed: $?";
    
}

sub create_lb {
    my @args = ("/home/midhun/projects/loadbalancer/create_loadbalancer.pl", "-a", $ENV{'RACKSPACE_ACCOUNT_NUMBER'}, "-n", $db_hostname, "-p", "3306");
    system(@args) == 0 or die "system @args failed: $?";
}
